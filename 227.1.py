# Chapter 10
# 101.5 Randomly Permuting a list
...
def permute(lst):
     ...
     for i in range(n -1):
         ...
         lst[i], lst[pos] = lst[pos], lst[i] # Sir my doubts:    mujhe yhe line samajh nhi aa rhi h
...
def faulty_permute(lst):
    ...
    for i in range(n -1):
        ...
        lst[i], lst[pos] = lst[pos], lst[i]    # Sir my doubts: Yhan par bhi samajh nhi aa rha h 
...
def run_test(perm, runs):
                    # Uses a permutation function to generate the permutations of the list [1,2,3]
                    # perm: the permutation function to test,   runs: the number permutations to perform

                    # The list to permute
    original = [1, 2, 3]

                    # Permutation_tally list keeps track of each permutation pattern
    permutation_tally = 6 * [0]  # Sir my doubts:   Sir mujhe es line ka matalab samajh nhi aa rha h    
    ...
    for i in range(runs):             # Run runs times
        ...
        permutation_tally[classify(working)] += 1          # Sir my doubts: Sir yhan par bhi samajh nhi aa rha h 
        report(permutation_tally)       # Report results

def main():
                    
        runs = 15   # Sir, maine yhan par value change kiya h 1000000 ki jagah 15 liya h  
