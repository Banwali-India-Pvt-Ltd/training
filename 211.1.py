...
def random_list():
                ...
                # The list's length is chosen pseudorandomly in the range 3-20
...
def selection_sort(lst,cmp):
    ...
    n = len(lst)
    for i in range(n - 1):
                       # Note i, small, and j represent positions withih lst
                       ...
                       # than lst[i]
          small = i
          for j in range(i + 1, n):    # See if a smaller value can be found later in the list
                                       # Consider all the elements at position j, where i < j < n.
               if cmp(lst[j], lst[small]):
                   small = j           # Found a smaller value
          if i != small:               # Swap list[i] and lst[small], if a smaller value was found
